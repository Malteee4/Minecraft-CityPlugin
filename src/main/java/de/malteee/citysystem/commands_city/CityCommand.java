...
public class CityCommand {
    ...
    private static HashMap<UUID, Location[]> confirmation = new HashMap<>();
    private static HashMap<UUID, City> confirmationCity = new HashMap<>();

    public static void register() {
        StringArgument cityArgument = new StringArgument("city");
        cityArgument.replaceSuggestions(ArgumentSuggestions.strings(c -> CitySystem.getCm().getCities().stream()
                .filter(City::isSpawnPublic).map(City::getName).toArray(String[]::new)));

        new CommandAPICommand("city")
                        ...
                        .executesPlayer((player, args) -> {
                            ...
                            if (currentArea.getType().equals(Area.AreaType.CITY) || currentArea.getType().equals(Area.AreaType.PLOT)) {
                                ...
                                player.sendMessage("§aYou're currently in §l" + city.getName() + "§r§a:\n" +
                                        ...
                                        "  §r§7residential plots: §o" + city.getPlots().size() + "\n" +
                        ...
                        .executesPlayer((player, args) -> {
                            ...
                            if (currentArea == null) {
                                ...
                                if (city.getOwner().equals(player.getUniqueId()))
                                    openCitySettings(player, city);
                                else
                                    player.sendMessage("§cYou can't open the settings of a city that's not yours!");
                            }else {
                                player.sendMessage("§cYou're currently not in a city!");
                            }
                        }))
                .withSubcommand(new CommandAPICommand("expand")
                        .executesPlayer((player, args) -> {
                            CityPlayer cPlayer = CitySystem.getCityPlayer(player);
                            if (cPlayer == null) return;
                            ...
                            int ar = (Math.max(loc1.getBlockZ(), loc2.getBlockZ()) - Math.min(loc1.getBlockZ(), loc2.getBlockZ())) * (Math.max(loc1.getBlockX(), loc2.getBlockX()) - Math.min(loc1.getBlockX(), loc2.getBlockX()));
                            ...
                            for (Area a : cityToExpand.getAreas())
                                space += a.getSurface();
                            if (ar + space >= cityToExpand.getStage().getMaxSize()) {
                                player.sendMessage("§cYou have reached your city size limit! Upgrade your stage to expand your city further!");
                                return;
                            }
                        ...
                        .withSubcommand(new CommandAPICommand("confirm")
                                .executesPlayer((player, args) -> {
                                    ...
                                    if (confirmation.containsKey(player.getUniqueId())) {
                                        ...
                                        player.sendMessage("§cPlease use §l/city expand §r§cfirst!");
                                    }
                                })))
                .withSubcommand(new CommandAPICommand("extensions")
                        .executesPlayer((player, args) -> {
                            CityPlayer cPlayer = CitySystem.getCityPlayer(player);
                            if (cPlayer == null) return;

                        }))
                .withSubcommand(new CommandAPICommand("extension"))
                .withSubcommand(new CommandAPICommand("spawn")
                        ...
                        .withSubcommand(new CommandAPICommand("setAccess")
                                .withArguments(new BooleanArgument("access"))
                                .executesPlayer((player, args) -> {
                                    CityPlayer cPlayer = CitySystem.getCityPlayer(player);
                                    if (cPlayer == null) return;
                                    boolean access = (boolean) args.get("access");
                ...
                .withSubcommand(new CommandAPICommand("delete")
                        .withPermission("CitySystem.city.delete")
                        .executesPlayer((player, args) -> {
                            CityPlayer cPlayer = CitySystem.getCityPlayer(player);
                            if (cPlayer == null) return;
                ...
                .withSubcommand(new CommandAPICommand("toggleBorder")
                        .executesPlayer((player, args) -> {
                            CityPlayer cPlayer = CitySystem.getCityPlayer(player);
                            if (cPlayer == null) return;
                            Area currentArea = cPlayer.getCurrentArea();
...
*/
    public static void openCitySettings(Player player, City city) {
